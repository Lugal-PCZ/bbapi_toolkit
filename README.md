...
## Configuration
Duplicate the _config.ini.example_ file as _config.ini_, and modify the settings to match the needs of your current project.


## Usage
Importing the module creates the client connection to Blackbaud’s APIs. Once created, pre-built functions provide a consistent interface to the API. The modules and functions in this package mirror the organization of Blackbaud’s API documentation: ```category/group/function``` _or_ ```category/function```.
...
roles = onapi.constituents.role.get_roles()
listresults = onapi.list.get_list(12345)  # Change this id to that of a list which you can access.
```
